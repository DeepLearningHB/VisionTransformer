...
from dataset.cub200 import get_cub_dataloader, get_cub_cskd_loader
from dataset.fgvc import get_fgvc_dataloaders,get_fgvc_cskd_loader
from dataset.cars import get_car_dataloader, get_cars_cskd_loader
#--------------
# utils 
#--------------
...
def dataloader(args):
    if args.data_type == 'cifar100':
        train_loader, valid_loader, _ = get_cifar100_dataloaders(
            batch_size=args.batch_size,
            num_workers=4,
            is_instance=True
        )
    elif args.data_type == 'CUB-200':
        train_loader, valid_loader, _ = get_cub_dataloader(batch_size=args.batch_size,
            num_workers=4,
            is_instance=True)
    elif args.data_type == 'FGVC':
        train_loader, valid_loader, _ = get_fgvc_dataloaders(batch_size=args.batch_size,
                                                           num_workers=4,
                                                           is_instance=True)
    elif args.data_type == 'CARS':
        train_loader, valid_loader, _ = get_car_dataloader(batch_size=args.batch_size,
                                                           num_workers=4,
                                                           is_instance=True)
    elif args.data_type == 'CUB_CSKD':
        train_loader, valid_loader, _ = get_cub_cskd_loader(batch_size=args.batch_size,
            num_workers=4,
            is_instance=True)
    elif args.data_type == 'FGVC_CSKD':
        train_loader, valid_loader, _ = get_fgvc_cskd_loader(batch_size=args.batch_size,
                                                           num_workers=4,
                                                           is_instance=True)
    elif args.data_type == 'CARS_CSKD':
        train_loader, valid_loader, _ = get_cars_cskd_loader(batch_size=args.batch_size,
                                                           num_workers=4,
                                                           is_instance=True)
